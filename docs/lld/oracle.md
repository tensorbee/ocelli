...
F-014 makes this row the worked permanent quirk capture. The record in
`corpus/quirks.json` names the focused comparator test as its executable
regression and binds it to the synthetic recipe, manifest row, PS3.3 fixture
and three observed mutations. The tier-independent `quirks` gate validates
the source and literal links in the CI floor without pretending a GPU
comparison ran there. The non-floor `quirk-mutations` gate executes its three
checker-owned edits in the CI corpus-tooling job, binds the regression command
to the attribution mutation and requires every fixed boundary to go red for
the recorded reason. The full oracle and corpus gates
remain the local proof that the named generated bytes reach decode,
presentation, readback and comparison under deviation D-04.
