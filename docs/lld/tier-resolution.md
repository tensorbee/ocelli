...
| `exactly_one_of_the_six_combinations_permits_compute` | `caps.rs` | The same predicate, counted rather than enumerated. An `||` permits FOUR of six, not five: three rows carry `caps.compute`, two carry `Tier::A`, and the union of `3 + 2 - 1` is four, measured by running the test against the `||` and reading `left: 4` from the assertion. Ignoring the tier permits three, ignoring `caps.compute` permits two, and exactly one is correct, so 1, 2, 3 and 4 still separate all four implementations |
