...
**F-IDs that contributed:** F-009, F-013, F-014, F-016, F-026, F-030, F-X006, F-X007, F-X012, F-X013
...
## Generating one case, and why the selector exists

`corpus_synth.py --case NAME` writes one case and leaves every other generated
file alone. `--list-cases` prints the names it accepts.

This is not a convenience. `generate()` removes and rebuilds the whole
synthetic and syntax layer, so it re-encodes every externally encoded row with
whatever version of that encoder is installed today. **When F-030 needed to add
three rows, `--tool-versions` reported OpenJPH at 0.31.0 against the 0.26.3 the
manifest was built with.** A full regeneration would have moved the three
`htj2k_*` digests inside a colour story, which is a toolchain bump wearing
another story's commit message. `--tool-versions` makes that drift visible and
`--case` makes it avoidable.

`--write-manifest` re-reads every digest from the files already on disk, so
adding a case through the selector leaves every existing row byte-identical.
F-030's own run added exactly three rows and moved no other digest, and the
OpenJPH drift is still there, still reported, and still someone's deliberate
decision to make rather than a side effect of an unrelated story.

`case_callables` is one list read by both the full run and the selector, so a
case cannot be generated by one and missing from the other. A test asserts that,
and a second asserts every synthetic case has a `CATEGORIES` row, because a case
without one writes its file and then fails at `--write-manifest`.

`scripts/quirk_check.py` follows reachability from `generate` transitively
rather than reading its direct calls, for the same refactor. A direct-call
reading reported the one permanent quirk's case, `case_sigmoid_width_half`, as
unreachable while it was still generated, which is a guard failing on a refactor
rather than on a defect. The rule still discriminates: dropping that case from
`case_callables` turns the check red, which was verified by mutation rather than
reasoned about.

## F-030's colour rows

Three, added through the selector above.

| Row | What it traps |
|-----|---------------|
| `synthetic/sc_palette_color.dcm` | the descriptor's first mapped input, non-zero at 10, with stored values both below and above it |
| `synthetic/sc_palette_color_16.dcm` | an entry count declared 0, meaning 65,536, which is the only legal spelling because 65,536 does not fit in a `US` |
| `syntax/jpegls_lossless_rgb8.dcm` | the multi-component JPEG-LS row Appendix A gate A2 recorded as owed |

The corpus had no `PALETTE COLOR` case at all before this, so neither palette
trap had a file behind it. `scripts/corpus_check.py` already knew the
photometric value and validated one if present, and nothing required one to
exist.

The palette channels are three different functions of the index, `i`, `255 - i`
and `3i mod 256`. Equal channels would satisfy a red-green-blue transposition
and a monotone triple would satisfy an off-by-one in every channel at once, so
neither could be caught by anything reading the case.
...
- **No 8-bits-per-entry palette LUT Data.** Both palette rows F-030 added
  declare 16 bits per entry, where one entry per 16-bit word is unambiguous.
  PS3.3 C.7.6.3.1.6's packing for 8-bit LUT Data is a PS3.5 encoding question
  that belongs to the parser story, and baking a guess about it into a manifest
  digest would make the guess look like evidence. The 8-bit **descriptor** path
  is proven by `crates/ocelli-pixel/tests/palette.rs`, which exercises
  `LutDescriptor` directly and depends on no wire packing.
