...
| D-20 | §15.2 says the wasm codec feature set selects `openjp2` explicitly and disables defaults so Rayon is absent | F-026 uses exactly pinned `ritk-codecs` 0.6.0 for JPEG 2000 Part 1 on native and wasm. Its JPEG 2000 path contains no unsafe Rust and uses no Rayon, but its manifest unconditionally reaches `jpeg-decoder` with default features and therefore resolves Rayon even though that unrelated JPEG module is never called | F-X006 proved `openjp2` 0.6.1 does not link as published for `wasm32-unknown-unknown`, traps on both Part 1 control codestreams after an allocator shim, and has a native null-deallocation defect. Reusing it would contradict measured evidence. `ritk-codecs` implements both reversible 5/3 and irreversible 9/7, contains no unsafe Rust in the published package, and the S08 probe compiles it for native and wasm. The transitive Rayon dependency is an honest cost of the published crate rather than a hidden feature choice. F-026 may register neither UID until corpus conformance and size checks pass, and it uses only the single-threaded JPEG 2000 entry point. | F-026 |
| D-21 | §21's `Decoder::decode` comment says decoding one frame into caller-provided `out` must not allocate | Concrete JPEG and JPEG 2000 adapters may receive bounded library-owned decoded storage from safe third-party APIs, validate it completely, then copy it atomically into the caller-provided output. Registry dispatch and raw plus RLE decoders remain allocation-free per call | `jpeg-decoder`, `oxideav-mjpeg`, and `ritk-codecs` expose safe decode APIs that return owned output rather than accepting caller scratch. Reimplementing or reaching into codec internals to preserve the comment would enlarge the quietly-wrong pixel and safety audit surfaces. Decode runs in decode workers and never in the render loop. The allocation is measured by the standing `decode.frame` benchmark, no adapter may partially update caller output, and the public output ownership contract remains caller-provided. This deviation makes the cost observable instead of falsely claiming no allocation. | F-024, F-026 |
