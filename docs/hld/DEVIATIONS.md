...
| D-20 | §15.2 says the wasm codec feature set selects `openjp2` explicitly and disables defaults so Rayon is absent | F-026 uses an exact local path to the published `ritk-codecs` 0.6.0 package for JPEG 2000 Part 1 on native and wasm. The package is unchanged except that both Cargo manifests disable `jpeg-decoder` default features. Immutable digests outside the vendor tree bind the complete 74-file published-package inventory and complete patch-provenance record. Complete-byte hashes bind both manifests to the sole declared patch. Exact archive and VCS identity, both upstream licence texts, and no-Rayon native and wasm graphs are also enforced by the pins gate | F-X006 proved `openjp2` 0.6.1 does not link as published for `wasm32-unknown-unknown`, traps on both Part 1 control codestreams after an allocator shim, and has a native null-deallocation defect. Reusing it would contradict measured evidence. `ritk-codecs` implements reversible 5/3 and irreversible 9/7 with no unsafe Rust in the published package. The published manifest unnecessarily enables Rayon through an unrelated JPEG module. The minimal local manifest-only patch restores the HLD's required no-Rayon graph without translating source or changing codec behavior. Native, plain wasm and SIMD wasm execute the same production sample checks. | F-026 |
| D-21 | §21's `Decoder::decode` comment says decoding one frame into caller-provided `out` must not allocate | Concrete JPEG and JPEG 2000 adapters may allocate bounded library-owned decoded storage, validate it completely, then copy it atomically into the caller-provided output. An adapter may also make a bounded encoded-input copy when its safe third-party packet API requires owned bytes. Registry dispatch and raw plus RLE decoders remain allocation-free per call | `jpeg-decoder`, `oxideav-mjpeg`, and `ritk-codecs` expose safe decode APIs that return owned output rather than accepting caller scratch. The public `oxideav-core` packet constructor also requires owned encoded bytes, so F-024's `.51` path copies the compressed frame once before decode. Reimplementing or reaching into codec internals to preserve the comment would enlarge the quietly-wrong pixel and safety audit surfaces. Decode runs in decode workers and never in the render loop. These allocations are measured by the standing `decode.frame` benchmark, no adapter may partially update caller output, and the public output ownership contract remains caller-provided. This deviation makes the cost observable instead of falsely claiming no allocation. | F-024, F-026 |
