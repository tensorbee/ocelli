...
| D-18 | §15.2 declares `dicom = { version = "0.10", default-features = false }` and its feature note says to select `jpeg`, `rle`, `deflate` and `openjp2` explicitly | Use the 0.10 `dicom-object` and `dicom-transfer-syntax-registry` component crates directly, both with defaults disabled. F-016 enables only `deflate`, while pixel codec features remain owned by F-023 and its dependants | The 0.10 umbrella crate exposes `inventory-registry`, `ul`, `pixeldata`, `image` and `ndarray`, not the four features the HLD names. It also declares `dicom-transfer-syntax-registry` without disabling that crate's `rayon` and `simd` defaults, so the written umbrella dependency cannot satisfy the feature instruction beside it. Direct components are the dicom-rs supported alternative and preserve the intended wasm-safe control. Deflate belongs here because PS3.5 A.5 compresses the entire data set. Encapsulated pixel decoding does not, and remains outside F-016. | F-016 |
