...
| D-24 | §20 gives `pub fn insert(&mut self, k: K, v: V) -> Vec<(K, V)>`, whose own doc comment reads "Returns the entries evicted to make room, so the caller can emit events" | `Lru::insert` returns `Admission<K, V> { evicted: Vec<(K, V)>, displaced: Option<V>, refused: Option<(K, V)> }`. `evicted` is §20's vector unchanged and carries only entries removed to fit the incoming one. `displaced` is the previous value under a repeated key. `refused` is the incoming entry handed back because the budget cannot hold it, which `Lru::would_admit` decides and which is `bytes` above the whole budget or a budget of zero, and it is populated before anything is evicted, so a refusal never costs the caller entries it still had room for | Three outcomes reach the caller and §20's comment describes one of them. A value replaced under an existing key was not evicted to make room, and an entry the budget cannot hold, whether because it is too large or because the tier was given no budget at all, was never admitted, so neither belongs in a vector documented as evictions. The distinction is not cosmetic: F-032 surfaces these as JS events and they are three different events, and F-034's memory-pressure telemetry counts evictions, which a replacement would inflate. Collapsing all three into the vector leaves the caller distinguishing a refusal by comparing the returned key against the key it just passed in, which is a rule no signature states. The third option, admitting an oversized entry and letting `used` exceed `budget`, was rejected in the S11 design round: a budget that reports more bytes in use than it permits is the defect `docs/sprints/CURRENT_SPRINT.md` names for this story, and it makes `Budgeted::bytes` a number that does not describe memory. The common path is unaffected, because an insert that fits with nothing displaced returns three empty fields and `Vec::new` does not allocate | F-031 |
