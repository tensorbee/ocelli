...
**`--check` cannot pass and has not been able to since the bootstrap.** The
tracked file has been hand-edited by every sprint since, fifteen `F-X` stories
have been added that were never in the spreadsheet, and the S03 review's fourth
pass removed the generated summary block outright, because every headline
figure in it was wrong and nothing regenerated it. An earlier version of this
docstring and of the header text below named `scripts/gen_backlog_summary.py`
as the regenerator. **No such script has ever existed in this repository.**
`--check` also needs a private source document and openpyxl, so it refuses
before it reaches any of that, which is why nobody noticed.
...
def render_backlog(rows: list[dict]) -> str:
    ...
    w("(consistency audit). The row table below is the record.")
    w("")
