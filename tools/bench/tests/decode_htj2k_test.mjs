...
import { parseHtJ2k } from "../src/runners/decode_transfer_syntax_htj2k.mjs";
...
test("HTJ2K result retains measured evidence", () => {
  const result = parseHtJ2k(record());
...
test("HTJ2K result requires the recorded warm-up", () => {
  const parsed = JSON.parse(record());
  delete parsed.warmup_iterations;
  assert.throws(() => parseHtJ2k(JSON.stringify(parsed)), /warm-up iteration/);
});

test("HTJ2K result requires the runner iteration contract", () => {
  for (const changes of [
    ...
    assert.throws(() => parseHtJ2k(record(changes)), /iteration/);
  }
});

test("HTJ2K iteration provenance counts timing samples", () => {
  assert.throws(
    () => parseHtJ2k(record({ iterations: 30 })),
    /31 timing samples/,
  );
});

test("HTJ2K result requires the four-decode normalized sample", () => {
  ...
  for (const candidate of [
    ...
    assert.throws(() => parseHtJ2k(candidate), /decodes per timing sample/);
  }
});

test("HTJ2K result refuses a missing or malformed range", () => {
  const missing = JSON.parse(record());
  ...
  for (const candidate of [
    ...
    assert.throws(() => parseHtJ2k(candidate), /finite observed range/);
  }
});

test("HTJ2K result refuses non-positive range endpoints", () => {
  for (const range_ms of [[-2, -1], [0, 0.6]]) {
    assert.throws(() => parseHtJ2k(record({ range_ms })), /positive observed range/);
  }
});

test("HTJ2K result refuses a reversed range", () => {
  assert.throws(
    () => parseHtJ2k(record({ range_ms: [0.6, 0.4] })),
    /ordered observed range/,
  );
});

test("HTJ2K result requires the range to enclose the median", () => {
  for (const range_ms of [[0.2, 0.4], [0.6, 0.8]]) {
    assert.throws(() => parseHtJ2k(record({ range_ms })), /enclose the median/);
  }
});

test("HTJ2K result refuses missing duration or checksum evidence", () => {
  assert.throws(() => parseHtJ2k(record({ value: 0 })), /positive finite duration/);
  assert.throws(() => parseHtJ2k(record({ checksum: 0 })), /output checksum/);
});

test("the HTJ2K baseline is the median of its complete calibration series", () => {
  ...
  const entries = Object.values(baseline.host_classes).flatMap((host) =>
    Object.entries(host.subjects)
      .filter(([id]) => id === "decode.transfer_syntax.htj2k")
      .map(([, entry]) => entry));
  assert.equal(entries.length, 1, "expected exactly one HTJ2K baseline");
  const [entry] = entries;
  ...
  assert.equal(entry.tolerance, 0.05,
    "the HTJ2K tolerance differs from the documented 5 per cent");
  assert.match(entry.tolerance_provenance, /retained 5 per cent/,
  ...
  // Three codec subjects now carry a calibration paragraph with the same
  // wording, and a non-global regex returns the first match, so the search is
  // scoped to this subject's paragraph.
  const whole = readFileSync(repoPath("docs", "lld", "benchmarks.md"), "utf8");
  const marker = "F-027 adds `decode.transfer_syntax.htj2k`";
  assert(whole.includes(marker), "the LLD does not introduce the HTJ2K subject");
  const lld = whole.slice(whole.indexOf(marker));
  assert.match(lld, /The retained 5 per cent is the\s+smallest declared symmetric band/,
...
test("HTJ2K timing provenance names the normalized batch clock", () => {
  ...
  const entry = Object.values(baseline.host_classes)
    ...
    .find(([id]) => id === "decode.transfer_syntax.htj2k")?.[1];
  assert(entry, "expected the HTJ2K baseline");
  ...
  const runner = readFileSync(
    repoPath("tools", "bench", "src", "runners", "decode_transfer_syntax_htj2k.mjs"),
    "utf8",
  );
