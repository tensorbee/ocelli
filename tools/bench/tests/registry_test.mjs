...
test("the Roadmap table is a status table too", () => {
  ...
  //
  // The Roadmap table has the SAME column count as the milestone tables and
  // different headers. Both are eight wide, measured with
  // `awk -F'|' 'NR==95 || NR==399 {print NR": "NF-2}' docs/sprints/BACKLOG.md`,
  // which prints `95: 8` and `399: 8`. What differs is what the columns mean:
  // a milestone row is `Sprint, Story, Layer, Est, Depends on` where a Roadmap
  // row is `Phase, Epic, Story, Layer, Est`. The status is therefore taken by
  // position from the END, as the second-to-last cell, which is stable across
  // both shapes where a count or an index from the front is not. Both shapes
  // are here so the reader is not reading position off one example.
  const backlog = [
...
test("a `## ` section ends the status table above it", () => {
  ...
  //
  // The consequence is not hypothetical, which is why the fixture below puts
  // the `## ` section AFTER a status table rather than before it. Moving the
  // recorded-defects table below the milestones, an ordinary reordering of a
  // document nothing forbids reordering, would hand `backlogStatuses` rows
  // whose second-to-last cell is defect prose, and F-145 would acquire the
  // status "declared dependency E4.9 does not exist". The driver throws on an
  // unknown status rather than labelling a subject wrongly, so the failure is
  // loud, and that caps the severity rather than making the line optional.
  const backlog = [
    "### M1, Foundations",
    "",
    "| F-ID | Epic ref | Sprint | Story | Layer | Est | Depends on | Status |",
  ...
  assert.equal(
    statuses.has("F-145"),
    false,
    "a row under a `## ` section after a status table was read as a status",
  );
