// One real `.91` Decoder::decode call over the synthetic corpus frame.

import { spawn } from "node:child_process";
import { access } from "node:fs/promises";
import { repoPath } from "../paths.mjs";

export const id = "decode.transfer_syntax.jpeg2000";
const BINARY = repoPath("target", "release", "examples", "decode_jpeg2000");
...
export function parseJpeg2000(text) {
  const parsed = JSON.parse(text);
  if (!Number.isFinite(parsed.value) || parsed.value <= 0) {
    throw new Error("JPEG 2000 benchmark produced no positive finite duration");
  }
  if (!Number.isInteger(parsed.iterations) || parsed.iterations !== ITERATIONS_KEPT) {
    throw new Error("JPEG 2000 benchmark iteration count does not match the runner contract of 31 timing samples");
  }
  if (!Number.isInteger(parsed.warmup_iterations) ||
      parsed.warmup_iterations !== WARMUP_ITERATIONS) {
    throw new Error("JPEG 2000 benchmark warm-up iteration count does not match the one-call runner contract");
  }
  if (!Number.isInteger(parsed.decodes_per_sample) ||
      parsed.decodes_per_sample !== DECODES_PER_SAMPLE) {
    throw new Error("JPEG 2000 benchmark decodes per timing sample do not match the four-call runner contract");
  }
  if (!Array.isArray(parsed.range_ms) || parsed.range_ms.length !== 2 ||
      !parsed.range_ms.every(Number.isFinite)) {
    throw new Error("JPEG 2000 benchmark produced no finite observed range");
  }
  const [minimum, maximum] = parsed.range_ms;
  if (minimum <= 0 || maximum <= 0) {
    throw new Error("JPEG 2000 benchmark produced no positive observed range");
  }
  if (minimum > maximum) {
    throw new Error("JPEG 2000 benchmark produced no ordered observed range");
  }
  if (parsed.value < minimum || parsed.value > maximum) {
    throw new Error("JPEG 2000 benchmark observed range does not enclose the median");
  }
  if (!Number.isSafeInteger(parsed.checksum) || parsed.checksum <= 0) {
    throw new Error("JPEG 2000 benchmark produced no output checksum");
  }
  return parsed;
}
...
export async function run({ build = true }) {
  if (build) {
    await child(repoPath("bin", "ocelli.sh"), ["cargo", "build", "--release", "-p", "ocelli-codec", "--example", "decode_jpeg2000"]);
  } else {
    await access(BINARY);
  }
  const result = parseJpeg2000(await child(BINARY, []));
  return {
    value: result.value,
    detail: {
      ...
      output_checksum: result.checksum, transfer_syntax: "1.2.840.10008.1.2.4.91",
      rows: 64, columns: 96, bits_stored: 16, profile: "release",
      fixture: "synthetic manifest-backed jpeg2000_lossy corpus frame",
