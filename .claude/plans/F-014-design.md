...
5. F-012 owns the explicit candidate comparison and coverage record that a
   future renderer quirk can cite. F-X021 owns making that candidate verdict a
   required gate after the renderer exists. F-014 can build the capture path
   now by using F-X012's already completed, independently checked reference
   quirk as its worked example.
...
5. Demonstrate the full path with F-X012's deliberately small
   `synthetic/ct_sigmoid_width_half.dcm`. Its centre, width and display values
   are literal fixture data derived from DICOM PS3.3 C.11.2.1.3.1. The case is
   generated below `corpus/data`, represented by a manifest row and exercised
...
- unsafe: none
- Tier A (WebGPU): n/a. The worked example is reference and comparator evidence
- Tier B (WebGL2): n/a
- Tier C (CPU): n/a
...
| fixture | Width 0.5 is valid for SIGMOID and the literal display values follow DICOM PS3.3 C.11.2.1.3.1 independently of either renderer | existing `scripts/tests/test_corpus_synth.py` fixture, linked by the capture record |
...
None. The HLD, D-04, D-05, the narrowed F-012 plan and F-X012's existing
independent fixture settle the design.
