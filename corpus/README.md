...
This section is for a public or synthetic case whose provenance already
permits local use. A field-bug report follows the capture path below. It never
puts the reporting file under `corpus/data`.

To add a permitted case:
...
## Capturing a field quirk

Every field bug becomes a permanent fixture under HLD section 11, but the
field input never becomes that fixture. No input file, identifier, metadata
dump, screenshot, pixel excerpt or digest of the input enters a prompt,
tracked artefact or log. If the defect cannot be reduced without retaining
that material, it is not captured in this repository.

The permanent capture is one entry in `corpus/quirks.json`. Follow this order:

1. Describe only the non-sensitive defect shape and reconstruct it with
   synthetic constants in a callable `scripts/corpus_synth.py` case.
2. Generate below ignored `corpus/data`, then add the generator-owned manifest
   row. Never track the generated DICOM.
3. Write expected values independently from DICOM PS3.3, PS3.16 or a literal
...
5. Run `bin/ocelli.sh gate quirks`, `bin/ocelli.sh gate quirk-mutations`,
   ...
   uses the locked DICOM interpreter and cargo in the CI corpus-tooling job.
   The latter two prove the generated bytes and render boundary locally under
   deviation D-04.

`scripts/quirk_check.py` connects the registry entry to the callable generator,
...
for patient data and tracked DICOM.
